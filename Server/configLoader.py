...
def StoreConfig(config, configName):
    conf = open(os.path.join(_ConfigPath, configName).replace("\\","/"), "w") 
    json.dump(config, conf, indent=4)
    conf.close()

def LoadConfig(configName):
    conf = open(os.path.join(_ConfigPath, configName).replace("\\","/"), "r")
    config = json.load(conf)
    conf.close()
    return config

def doesConfigExist(configName):
    return os.path.exists(os.path.join(_ConfigPath, configName).replace("\\","/"))